...
**Last Updated:** 2026-05-21 (Phase 2.6 Theme 1 — `KUZO_HOME` env override + shared paths helpers — merged via PR #45 on 2026-05-21 at `9c8149c`. 3 rounds, all ship across all 3 specialists every round. Round 1: 0/3 advisory (env empty-string + comma/newline rejection + lint selector widening, all fixed in `87b1b08`). Round 2: 0/4 advisory (namespace-import `os.homedir()` lint coverage + `pluginFsPath` fs-arg validation + PR body correction, fixed in `528f8e6`; 1 advisory pushed back inline as spec §E.2 contract). Round 3: 0/3 advisory — impartial judge sub-agent called merge HIGH confidence; advisories tracked as **issue #46** for Theme 2 sweep (`kuzoHome()` fallback fs-arg assertion + dead CLI `kuzoHome` re-export + JSDoc drift). New `packages/core/src/paths.ts` exports the 8 §E.2-specified helpers (`kuzoHome`, `pluginsRoot`, `credentialsFilePath`, `consentFilePath`, `auditFilePath`, `tufCacheDir`, `attestationsCacheDir`, `kuzoHomeLockPath`) via the `./paths` subpath on `@kuzo-mcp/core`. 7 sites refactored (consent / audit / plugin-resolver / plugin-process / provenance/verify / CLI plugins/paths.ts / refresh-trust-root). ESLint `no-restricted-syntax` rule covers all 6 drift shapes (3 syntactic forms × named/namespace import). Sandbox-arg hardening: `kuzoHome()` rejects empty-string and `,`/`\n`; `assertNoFsArgInjection()` applied to both arms of `--allow-fs-read=` interpolation. Zero behavior change at defaults. **Next:** Theme 2 — A.1–A.4 Storage primitives (cipher, key providers, store interface + impl).)
...
**Locked next-session plan (updated 2026-05-21):**
...
2. **IMMEDIATE NEXT — Theme 2: A.1–A.4 Storage primitives** (cipher, key providers, store interface + impl). Spec §A.1–A.4. Uses unit tests against tmpdir + `InMemoryKeyProvider` test double. **Pre-Theme-2-merge sweep:** address issue #46 (Theme 1 round-3 carry-over) — either folded into the Theme 2 PR or as a tiny pre-Theme-2 commit. Per round-3 takeaway, the parent-eager-decrypt model means the store needs to be ready before any plugin loads.

3. **Then Theme 3 onwards** per the spec's §0 build order:
...
1. **Start at Theme 2 — A.1–A.4 Storage primitives** (cipher, key providers, store interface + impl). Read `docs/credentials-spec.md` §A.1–A.4. Open a new branch `phase-2.6/storage-primitives` (or per-Theme convention). Address issue #46 (Theme 1 round-3 carry-over) either inside the Theme 2 PR or as a tiny pre-Theme-2 commit on the same branch:
   - Assert `kuzoHome()` fallback branch (`homedir() + ".kuzo"`) against `assertNoFsArgInjection()` so the L161 comment in `plugin-process.ts` becomes accurate and every caller is protected.
   - Drop dead `kuzoHome` re-export in `packages/cli/src/commands/plugins/paths.ts:23`.
   - JSDoc drift sweep on `audit.ts:53` and similar in consent/verify (phrasing references stale `~/.kuzo` instead of helper functions).

   Theme 2 proper per §A.1–A.4:
   - Cipher primitives (AES-256-GCM per spec).
   - Key providers — `KeychainKeyProvider` (R5 inert constructor — no prompt on instantiate) + `PassphraseKeyProvider` (KUZO_PASSPHRASE env) + `InMemoryKeyProvider` (test double).
   - Store interface (`CredentialStore`) + filesystem impl reading/writing `credentialsFilePath()` (already pre-positioned in Theme 1's paths.ts).
   - Unit tests against tmpdir + `InMemoryKeyProvider`.
   - Quality gate: `pnpm install && pnpm build && pnpm typecheck && pnpm lint && pnpm test:parity` clean.
2. **Continue Theme 3 onwards** per the build order above. Each Theme is its own PR; the Theme 8 migrate PR ships alone with extra reviewer focus per round-4 advisory.

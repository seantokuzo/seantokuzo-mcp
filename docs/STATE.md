...
**No active phase.** Phase 2.5e fully closed (2026-05-03 — see "First real npm release" entry above). Plan flipped 2026-05-04 once we tried to walk the QA flow and discovered the credential-provisioning surface is missing entirely (no `kuzo init`, no inline prompt, plaintext-on-disk in two places, no isolation env knobs beyond `KUZO_PLUGINS_DIR`). That's a distribution blocker, so QA bake-time is gated on it.

**Locked next-session plan (agreed 2026-05-04):**

1. **NEXT SESSION — Credential research/spec session.** Read `docs/credentials-spec-brief.md` first (it's the input). Read `docs/SECURITY.md` §6 for the already-shipped broker design. Produce `docs/credentials-spec.md` modeled on `docs/2.5e-spec.md` — implementation-ready, parts A–F, locked decisions table, gotchas, cutover plan. **No code this session — spec only.** Brief enumerates 16 open questions with recommended defaults; spec session lands them.
2. **AFTER SPEC LANDS — Credential implementation phase.** Working title "Phase 2.6 — Credential Storage & Provisioning" (spec session locks the final name). Build per the spec's part-by-part order. Acceptance criteria includes end-to-end smoke against real keychain on macOS + Linux.
...
**On a fresh session, when user says "next":** start at step 1 unless they redirect. Don't skip ahead unless explicitly asked. **Step 1 is a research/spec session — do not write code.**

...
8. **IMMEDIATE NEXT — Themes 8–9** per the spec's §0 build order:
   - Theme 8: B.4 — `kuzo credentials migrate` (HIGHEST-RISK PR — symlink-safe + dotenv parser + step-3-substep ordering + read-back-verify + post-rewrite redaction-verify + storeSnapshot rollback per round-1/2/4 advisories). The 7 write-side `AuditAction` slots (`credential.migrated`/`.migration_partial` etc.) are already reserved (Theme 6); the migrate exit codes (60–63, 74–77) are already in `CRED_EXIT` (Theme 7). Lands the §B.4 `--source`/`--dry-run`/`--force-source` flags + `~/.claude/settings.json` + `.env` import with atomic redaction.

...
**Last Updated:** 2026-05-22 (Phase 2.6 Theme 2 — Storage primitives — merged via PR #47 on 2026-05-22 at `df6875d`. 2 rounds, impartial judge sub-agent called merge HIGH confidence after round 2. Round 1: synthesizer rolled up as 0/2 advisory but Correctness lane posted 1 BLOCKING inline (test files excluded from typecheck) without a JSON sentinel — silent under-counting. Real round-1 was 1 blocking + 2 advisory across specialists. All fixed in `da6357a`: (a) reverted `tsconfig.json` exclude (tests typecheck again) + moved tarball filtering to `packages/core/package.json#files` negation patterns (`.npmignore` is silently ignored when `files` is present per npm spec), (b) relocated `InMemoryKeyProvider` to a new `./credentials/testing.ts` published via the `@kuzo-mcp/core/credentials/testing` subpath off the main barrel, (c) Architecture's mode-method coupling advisory **pushed back as won't-fix** (YAGNI for closed 4-provider set per spec §A.5; not re-raised in round 2). Round 2: all 3 specialists ship; single cosmetic JSDoc advisory on `generationFilePath` softened in `e836d6e`. **New code surface:** `packages/core/src/credentials/` with `cipher.ts` (AES-256-GCM + `KCR1` header, dual rollback defense via AAD = literal header bytes AND explicit `expectedGeneration` check), `errors.ts` (`KeyProviderError` + `CredentialStoreError` with `.code` discriminant), `key-provider.ts` (KeyProvider interface + KeychainKeyProvider via `@napi-rs/keyring@1.3.0` exact-pin / PassphraseKeyProvider scrypt N=2^17 r=8 p=1 maxmem=256MiB / NullKeyProvider env-override sentinel), `testing.ts` (InMemoryKeyProvider test double, quarantined behind `./credentials/testing` subpath + NODE_ENV/KUZO_TEST guard), `store.ts` (`EncryptedCredentialStore` impl — lazy decrypt, atomic tmp+fsync+rename writes, generation-persists-first per §A.3, KEY_LOST + CORRUPTED state surfacing), 58 node:test cases via `tsx`. AuditAction union extended with three lifecycle variants only (`credential.passphrase_consumed`, `credential.store_unlocked`, `credential.store_locked`); write-side variants deliberately deferred to Theme 5/6 per spec §0 build-order. Workflow infra debt surfaced: Tier 3 deep review failed bot-allowlist check → tracked as **issue #48**. **Next:** Theme 3 — A.6–A.7 CredentialSource + env-override collection (bridges store + env, pure logic).)
...
3. **IMMEDIATE NEXT — Theme 3: A.6–A.7 CredentialSource + env-override collection** (bridges store + env, pure logic, easy to test). Spec §A.6 (`CredentialSource` class merging env-overrides with the store) + §A.7 (`collectEnvOverrides` + `scrubProcessEnv`). Build on top of the Theme 2 primitives via the `@kuzo-mcp/core/credentials` subpath. No boot wiring yet — Theme 4 (server.ts) consumes both. Spec also describes the `KUZO_TOKEN_<NAME>` legacy-plus-namespaced envvar pattern + the `KUZO_PASSPHRASE` + `KUZO_NO_ENV_SCRUB` always-scrub list (round-4 A2). Unit-testable with node:test against `process.env` mocks.

4. **Then Theme 4 onwards** per the spec's §0 build order:
...
1. **Start at Theme 3 — A.6–A.7 CredentialSource + env-override collection.** Read `docs/credentials-spec.md` §A.6 + §A.7. Open a new branch `phase-2.6/credential-source` (or per-Theme convention). Pure-logic theme — no I/O, no boot wiring yet, easy to test against `process.env` mocks.

   Theme 3 surface per §A.6–A.7:
   - `packages/core/src/credentials/source.ts` — `CredentialSource` class with constructor `(store: CredentialStore, envOverrides: Record<string, string>)`. Methods: `get(key)` (env-override wins, then store, then undefined), `has(key)`, `extractForPlugin({ required, optional })` (drop-in replacement for `ConfigManager.extractPluginConfig` — same shape, returns `{ config: Map, missing: string[] }`). Use the `isCredentialCapability` type guard from `@kuzo-mcp/types` (round-4 B8) — should already exist post-Theme 0 manifest bake; verify.
   - `packages/core/src/credentials/env-overrides.ts` — `collectEnvOverrides(declaredEnvNames: ReadonlySet<string>)` returns `Record<string, string>` with the dual-pattern lookup (plain `GITHUB_TOKEN` AND `KUZO_TOKEN_GITHUB_TOKEN`; namespaced wins). Plus `scrubProcessEnv(scrubKeys, auditLogger?)` returning `ScrubProcessEnvResult` with `killSwitchActive` / `scrubbedCount` discriminants. `ALWAYS_SCRUB = ["KUZO_PASSPHRASE", "KUZO_NO_ENV_SCRUB"]` per round-4 A2. Pure function — auditLogger threaded as arg, not constructor.
   - Tests: `source.test.ts` + `env-overrides.test.ts` against tmpdir-backed `EncryptedCredentialStore` (or just env-override-only flows). Cover: env-override-precedence, store-fallback, missing-required-tracking, optional-silent-omission, `KUZO_TOKEN_<NAME>` overrides plain, kill-switch behaviour, always-scrub items unaffected by kill-switch.
   - Subpath additions to `packages/core/package.json` exports map (if needed — or keep these inside `./credentials`).
   - Quality gate: `pnpm install && pnpm build && pnpm typecheck && pnpm lint && pnpm test` clean.

2. **Continue Theme 4 onwards** per the build order above. Each Theme is its own PR; the Theme 8 migrate PR ships alone with extra reviewer focus per round-4 advisory.

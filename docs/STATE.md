...
**Last Updated:** 2026-05-23 (Phase 2.6 Theme 4 — boot sequence + scrub + key-provider selection — merged via PR #50 on 2026-05-23 at `5f43f06`. **4 rounds, judge merge HIGH confidence at the hard cap.** Verdict trend: SHIP 0/7 → SHIP 0/3 → SHIP 0/4 → SHIP 0/3 across the 4 rounds — every round shipped with zero blocking issues. Fix commits: `fd8f998` (round-1 7 advisories), `ca09f4b` (round-2 1 fix + 1 defer + 1 pushback), `e6bde7f` (round-3 3 fixes + 1 pushback), `4d7bd9a` (round-4 1 fix + 2 pushbacks). Round-4 verdict synthesizer crashed on `actions/checkout@v4` git auth ("could not read Username for 'https://github.com': terminal prompts disabled") — separate CI infra glitch on the synth job only; all 3 specialists shipped (Arch 0/0, Correctness 0/0, Security 0/3 all defense-in-depth per the specialist's own rationale). Tier 3 deep review auto-escalated again on large-diff (>500 lines) — label applied — and again hit issue #48's bot-allowlist failure on all 4 deep specialists. Standard CI (Lint/Typecheck/Build/Install parity/Provenance smoke) all green; rollup MERGEABLE + label `CI Pass`. **New code surface:** `packages/core/src/server.ts` rewritten into exported `async runServer(options?)` with the pinned §C.1 step order (installExitGuard → ConfigManager → ConsentStore + AuditLogger → chooseKeyProvider + EncryptedCredentialStore both INERT → collectDeclaredCredentialEnvNames → collectEnvOverrides → scrubProcessEnv → CredentialSource → PluginLoader(…credentialSource) → loader.loadAll → freezePrototypes → MCP connect → shutdown hooks (loader → registry → credentialStore.close → server.close)). Self-invocation guard at file bottom (round-4 B2) — only auto-boots when this file is the process entry. `packages/core/src/key-provider-choice.ts` (new) — §A.5 precedence: `KUZO_DISABLE_KEYCHAIN=1` + `KUZO_PASSPHRASE` → Passphrase; `KUZO_DISABLE_KEYCHAIN=1` alone → Null; `KUZO_PASSPHRASE` alone → Passphrase; else Keychain. `packages/core/src/manifest-env-names.ts` (new) — §C.1 step 5 `collectDeclaredCredentialEnvNames(kuzoConfig)`: synchronously reads STATIC `package.json#kuzoPlugin.capabilities` + `optionalCapabilities` via `fs.readFileSync` only (invariant 6 — no plugin entry `import()`), filters via `kind === "credentials"`, applies `RESERVED_KUZO_ENV` denylist (KUZO_PASSPHRASE, KUZO_NO_ENV_SCRUB, KUZO_HOME, KUZO_DISABLE_KEYCHAIN, KUZO_PLUGINS_DIR, KUZO_TRUST_PLUGINS, KUZO_TRUST_ALL, KUZO_TRUST_LEGACY, KUZO_STRICT, KUZO_NODE_PERMISSIONS — defense-in-depth, §A.12 install-time gate in Theme 7 is the canonical defense). `packages/core/src/plugin-resolver.ts` adds `resolvePluginPackageDir(name, kuzoConfig)` — three-tier resolution (versioned → flat → walk-up-from-`import.meta.resolve`) returning package-dir for the static walker. `packages/core/src/loader.ts` accepts `credentialSource: CredentialSource` as 6th constructor arg, swaps `extractPluginConfig` → `extractForPlugin({required, optional})` filtered via `isCredentialCapability`, deletes `extractV2Config`. `packages/core/src/config.ts` drops `extractPluginConfig` per §C.7. `eslint.config.js` adds the §C.9 `no-restricted-imports` + `no-restricted-syntax` rule pair scoped to `packages/core/src/**/*.ts` with `ignores: [plugin-process.ts, paths.ts]` (round-3 widened from just server.ts + loader.ts); `allowTypeImports: true` lets `ipc.ts:10` keep `import type { ChildProcess }`. **Round-1 SEC catch (load-bearing fix):** narrowed `scrubProcessEnv` keys from `[...declaredEnvNames, ...Object.keys(envOverrides)]` to `[...declaredEnvNames]` — closed the `KUZO_TOKEN_PATH=evil` → `delete process.env.PATH` attack. **Pushbacks documented in PR comments:** changeset cadence (pre-1.0 batches at phase close, not per-Theme), `node:` prefix convention drift (new files match the dominant 32 vs 14 split, legacy 5 outliers are pre-Theme-1), ESLint dynamic-import coverage gap (reviewer's own "Low priority" + Theme 7 boundary), plugin-name path-traversal hardening (user-owned `kuzo.config.ts` not an attacker surface; Theme 7 install CLI is the boundary). **Next:** Theme 5 — C.10 + C.10.1 — Plugin-host audit emissions over IPC + rate-limit + log rotation. Must land BEFORE any write-side audit events.)
...
5. **IMMEDIATE NEXT — Theme 5: C.10 + C.10.1 — Plugin-host audit emissions over IPC + rate-limit + log rotation.** Must land BEFORE write-side credential audit events (the four read-side broker emissions in 2.5b are the only allowlisted child events; everything else routes through `audit.forged_action` per round-1/2 takeaways). New `packages/core/src/audit-partition.ts` exhaustiveness-check is required to prevent drift. Surface includes the IPC routing for plugin-host → parent audit emissions, the rate-limit (debounce / ceiling per second per plugin), and the log rotation (size-based or time-based per spec §C.10.1). Branch `phase-2.6/audit-ipc` (or per-Theme convention).

6. **Then Themes 6–9 onwards** per the spec's §0 build order:

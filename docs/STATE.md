...
**Last Updated:** 2026-05-22 (Phase 2.6 Theme 3 — CredentialSource + env-override collection — merged via PR #49 on 2026-05-22 at `2ee5080`. 1 round, judge merge HIGH confidence. All 3 Tier 2 specialists shipped 0 blocking / 0 advisory on round 1; zero inline review comments. Tier 3 deep review auto-escalated (large-diff >500 lines) and the label actually applied this time (unlike PR #43 where the synthesizer hallucinated escalation) — but all 4 Tier 3 specialists + the Deep verdict synthesizer hit the **issue #48** bot-allowlist infra failure ("Workflow initiated by non-human actor: claude (type: Bot)"), which is a physical workflow failure, NOT a Tier 3 verdict. Standard CI (Lint/Typecheck/Build/Install parity/Provenance smoke) + Tier 2 all green; rollup MERGEABLE + label `CI Pass`. **New code surface:** `packages/core/src/credentials/source.ts` (`CredentialSource` class — env-override-wins-over-store merge, `extractForPlugin({required, optional})` drop-in for `ConfigManager.extractPluginConfig` per spec §A.6, round-4 B4 no-schema-change locked in via separate arrays not `optional?` field), `packages/core/src/credentials/env-overrides.ts` (`collectEnvOverrides` dual-pattern with `KUZO_TOKEN_<NAME>` namespaced-wins + `scrubProcessEnv` with `ALWAYS_SCRUB = [KUZO_PASSPHRASE, KUZO_NO_ENV_SCRUB]` per round-4 A2, prefix-delete skipped for ALWAYS_SCRUB per N1, auditLogger threaded as fn-arg per B11). `packages/types/src/index.ts` adds `isCredentialCapability` type guard (round-4 B8 — handoff said "verify"; wasn't there; mirrors `isV2Plugin`). `AuditAction` union extended with `credential.scrub_disabled` only (write-side variants still deferred to Theme 6/7 per §0). 39 new node:test cases via `tsx` (16 source + 23 env-overrides), `test:credentials` script wires both — 97 total credential tests passing locally. Round-2+ auto-review fire-mechanism (manual `gh workflow run claude-code-review.yml`) unused — round 1 was clean. **Next:** Theme 4 — C.1–C.3 + C.9 — boot sequence rewrite + self-invocation guard + child_process ESLint rule.)
...
**Locked next-session plan (updated 2026-05-22):**
...
4. **IMMEDIATE NEXT — Theme 4: C.1–C.3 + C.9 — boot sequence rewrite + self-invocation guard + child_process ESLint rule.** Spec §C.1 (boot sequence — `loadDotenv` → synchronously read `package.json#kuzoPlugin.capabilities` to build `declaredEnvNames` → `collectEnvOverrides` → `scrubProcessEnv` BEFORE any plugin entry `import()` per invariant 6 → `chooseKeyProvider` per §C.2 → `EncryptedCredentialStore` construction → `CredentialSource` construction → parent-eager decrypt during `loader.loadAll()` → existing plugin load flow). Spec §C.2 selects KeyProvider (KUZO_PASSPHRASE → Passphrase; SSH/CI/headless detect → Passphrase; KUZO_NO_KEYCHAIN=1 → Null; else Keychain). Spec §C.3 swaps `loader.ts:312` call from `configManager.extractPluginConfig(required, optional)` to `credentialSource.extractForPlugin({required, optional})` (mechanical signature shift — both return `{config: Map, missing: string[]}`). Spec §C.9 adds ESLint `no-restricted-imports` for `child_process` outside the allowed core paths (`plugin-process.ts`, `plugin-host.ts`) — guards the boot-step invariant that nothing forks before scrub. Round-4 B2: self-invocation guard at server.ts bottom (`if (import.meta.url === pathToFileURL(process.argv[1]).href) runServer()`) — does NOT exist today. Parity test (`pnpm test:parity`) must stay green. Surface lands in `packages/core/src/server.ts` (rewrite into `runServer()`) + `packages/core/src/loader.ts` (one-line call-site swap, receives `CredentialSource` instead of `ConfigManager` for credential extraction) + new `packages/core/src/key-provider-choice.ts` (`chooseKeyProvider`) + `eslint.config.js` (the new rule). Branch `phase-2.6/boot-sequence` (or per-Theme convention).

5. **Then Themes 5–9 onwards** per the spec's §0 build order:
   ...
   - Phase close: docs + canary release. **Patch bumps across the board per the pre-1.0 cadence in `CLAUDE.md` / `PLANNING.md`** — types `0.0.1 → 0.0.2`, core/cli/plugins `0.0.2 → 0.0.3`. NOT 0.1.0 — we're still pre-QA. The `AuditLogger` rename breaking-export gets a CHANGELOG callout. First `0.1.0` is reserved for the post-QA "trusted for daily use" moment after we run `kuzo serve` for a stretch.
...
1. **Start at Theme 4 — C.1–C.3 + C.9 — boot sequence rewrite + self-invocation guard + child_process ESLint rule.** Read `docs/credentials-spec.md` §C.1 + §C.2 + §C.3 + §C.9. Open a new branch `phase-2.6/boot-sequence` (or per-Theme convention). This is the first Theme with **live boot wiring** — the parity test must stay green throughout.

   Theme 4 surface per §C.1–C.3 + C.9:
   - `packages/core/src/server.ts` — Rewrite into a `runServer()` async function. §C.1 step order is non-negotiable (invariant 6 — scrub MUST land before any plugin entry `import()`):
     1. `loadDotenv()` (existing) — populates `process.env` from `.env`.
     2. Synchronously walk all **enabled** plugin `package.json#kuzoPlugin.capabilities` (the static fields baked in Theme 0) and `optionalCapabilities` to build `declaredEnvNames: Set<string>` and `scrubKeys: string[]`. No plugin entry `import()` happens yet — `package.json` read is `fs.readFileSync` + `JSON.parse` per enabled plugin via the `plugin-resolver`.
     3. `const envOverrides = collectEnvOverrides(declaredEnvNames)` — from Theme 3.
     4. `const scrubResult = scrubProcessEnv(scrubKeys, auditLogger)` — BEFORE any further `import()`. The `--no-scrub` CLI flag in §C.1 step 7 reuses the same `credential.scrub_disabled` audit action with its own reason string.
     5. `const keyProvider = chooseKeyProvider(env, auditLogger)` — §C.2 selection logic in a new `packages/core/src/key-provider-choice.ts`: `KUZO_PASSPHRASE` set → Passphrase; SSH/CI/headless detect (`SSH_CONNECTION`/`CI`/headless DBUS) → Passphrase; `KUZO_NO_KEYCHAIN=1` → Null; else Keychain.
     6. `const store = new EncryptedCredentialStore({filePath: credentialsFilePath(), keyProvider, auditLogger, logger})`.
     7. `const credentialSource = new CredentialSource(store, envOverrides)`.
     8. Parent-eager decrypt happens implicitly during `loader.loadAll()` — first `source.get(any-declared-env)` triggers `store.get()` which lazy-decrypts (or stays empty if file absent).
     9. Existing plugin load flow continues with `credentialSource` threaded into the loader.
   - `packages/core/src/loader.ts` — Line 312 call-site swap. Loader constructor / `loadAll()` signature receives `CredentialSource` instead of (or alongside) `ConfigManager` for credential extraction. `ConfigManager` still owns plugin enable/disable + `loadDotenv()`. The `extractForPlugin({required, optional})` call filters capabilities via `isCredentialCapability` from `@kuzo-mcp/types`.
   - `packages/core/src/key-provider-choice.ts` (new, §C.2) — `chooseKeyProvider(env, auditLogger)` selection logic + the SSH/CI/headless detection.
   - `eslint.config.js` — Add `no-restricted-imports` block scoped via `files` to `packages/core/src/**` EXCEPT `plugin-process.ts` and `plugin-host.ts`, blocking `child_process` (and `node:child_process`) imports. Guards spec §C.1 invariant 6 + §C.9.
   - `packages/core/src/server.ts` bottom (round-4 B2): self-invocation guard `if (import.meta.url === pathToFileURL(process.argv[1]).href) runServer()` — does NOT exist today. Without it, importing `server.ts` for testing auto-boots the server.
   - Tests: extend the existing parity test if loader signature changes; otherwise no new test surface (Theme 5+ adds audit-partition tests). Manual MCP handshake smoke per the "MCP end-to-end smoke" recipe in `docs/STATE.md` "Known tactical detail".
   - Quality gate: `pnpm install && pnpm build && pnpm typecheck && pnpm lint && pnpm test:credentials && pnpm test:parity` all clean. **Parity is the load-bearing test for this Theme** — it exercises the full installed-mode boot.

   **Tripwire from Theme 3 Security lane** (load-bearing): `collectEnvOverrides` is permissive on unknown `KUZO_TOKEN_*` names — Theme 4 boot MUST NOT call it against an unsanitized `declaredEnvNames`. Theme 7's §A.12 reservation gate (`FIRST_PARTY_ENV_RESERVATIONS` + `RESERVED_SYSTEM_ENVS`) is the production defense at install time. Until Theme 7 lands, all first-party plugins are trusted not to declare adversarial env names — that's true today (verified at Theme 0 manifest-bake), so Theme 4 can ship without the gate.

2. **Continue Theme 5 onwards** per the build order above. Each Theme is its own PR; the Theme 8 migrate PR ships alone with extra reviewer focus per round-4 advisory.

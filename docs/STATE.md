...
1. **Start at Theme 6 — C.4–C.6 — Broker write-side audit events + shutdown hooks; retire module-level `clientFactories` singleton.** Read `docs/credentials-spec.md` §C.4 + §C.5 + §C.6, plus the §A.1 broker contract that bounds what's permissible. Open a new branch `phase-2.6/broker-write-audit` (or per-Theme convention).

   **Build-order context:** Theme 5 (audit IPC routing) just landed in `026fcc0` and is the structural prerequisite for ANY new write-side audit events. The trust boundary is in place; the `CHILD_PERMITTED_AUDIT_ACTIONS` allowlist is currently exactly the 4 read-side broker emissions. Every new write-side `credential.*` audit action this Theme adds is **parent-only** and MUST be added to `AUDIT_ACTION_PARTITION` in `packages/core/src/audit-partition.ts` at the same time as the union extension in `audit.ts` — the TS `Record<AuditAction, ...>` makes that a compile error if missed.

   **Theme 6 surface (high-level — read the spec for exact shape):**

   1. **Broker write-side events (spec §C.4).** Wire `credential.set` / `.deleted` / `.rotated` / `.migrated` / `.migration_partial` / `.wiped` / `.tested` emissions through the broker (CALL SITES are parent CLI, NOT the in-child broker — these are gated to the `kuzo credentials *` surface arriving in Theme 7). The broker's existing `auditLogger` arg-pattern (round-4 B11 from Theme 3) is the threading mechanism.

   2. **Shutdown hooks (spec §C.5 + §C.6).** Boot-order invariant 4 from Theme 4 (`credentialStore.close()` after `loader.shutdownAll()`) is already in place; Theme 6 extends with whatever store-side close behavior the spec requires (cache zeroing, file watchers, generation-file flush). Confirm against `docs/credentials-spec.md` §C.5–C.6 for the exact list.

   3. **Retire `clientFactories` module-level singleton (Theme 4 round-4 advisory A1).** `packages/core/src/credentials.ts` currently holds the GitHub/Jira client-factory map at module scope. Move it to constructor-injectable state on `DefaultCredentialBroker` so each broker instance can be tested in isolation. Keep the same first-party factory wiring at the construction site in `loader.ts` (or wherever the broker is built per Theme 5's plugin-host updates).

   4. **`AuditAction` union extensions.** Each new variant lands BOTH in the union in `audit.ts` AND in `AUDIT_ACTION_PARTITION` in `audit-partition.ts`. Failing to do the latter = TS compile error (the exhaustiveness check from Theme 5). All Theme-6 variants are parent-only.

   **Acceptance criteria (spec §F.1 relevant subset for Theme 6):** consult the spec — Theme 5 already shipped the audit infrastructure, so Theme 6's tests focus on the write-side emission paths + the singleton retirement (each broker instance has its own factories, no cross-contamination).

   **Quality gate:** `pnpm install && pnpm build && pnpm typecheck && pnpm lint && pnpm test:audit && pnpm test:credentials && pnpm test:parity` all clean. The `test:audit` script chained into root `pnpm test` was added in Theme 5 — make sure it stays in the chain.

   **Pre-Theme-6 carryover from Theme 5 round-4:**
   - `eslint.config.js` `no-restricted-imports paths:` block matches literal `./audit.js`; if Theme 6 introduces any new file in a hypothetical `plugin-host/` subdir, switch to `patterns:` regex for the import side. Currently deferred — `files:` glob already widened.
   - **Issue #52** (forgery-flood retention erosion) — spec §C.10 emits per-attempt forgery records by design; if Theme 6's surface ever touches the retention budget or rotation policy, evaluate whether aggregating forgery emissions becomes worthwhile.

   **Pre-Theme-6 carryover from Theme 4 round-4 SEC advisories (still deferred to Theme 7 install CLI):**
   - ESLint `child_process` rule has coverage gaps vs `import("…").then`, `createRequire(import.meta.url)("…")`, aliased identifier dynamic import. Reviewer themselves tagged "Low priority".
   - `resolvePluginPackageDir` joins `installedRoot` with the friendly plugin name without shape validation. Add a name-validation regex (`/^[a-z][a-z0-9-]{0,62}$/`) at the top of `resolvePluginPackageDir` + `resolvePluginEntry` + the install CLI entry point when Theme 7 lands.

2. **Continue Theme 7+ onwards** per the build order above. Each Theme is its own PR; the Theme 8 migrate PR ships alone with extra reviewer focus per round-4 advisory.

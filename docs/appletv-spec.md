...
## 2. Build order — FOUNDATION FIRST (resolved 2026-06-02)

The remote-transport capability is the strategic deliverable (it's the spine of the McGraw-Hill customer-facing-MCP pitch — email going out 2026-06-02); the AppleTV plugin is the POC/demo that rides on it. So build the **transport foundation first, validated with the EXISTING github/jira plugins** (plugin-agnostic — needs no pyatv and no capability-system changes), then layer OAuth/hosting, then AppleTV.

- **4a — Remote-transport foundation (strategic core).** Refactor `runServer` → `bootKuzo()` (run once) + exported `buildMcpServer(registry)` (per session), stdio path byte-identical (`§4.1`); build the **opt-in `@kuzo-mcp/server-http`** (Streamable HTTP transport + sessions, `§4.2`); `kuzo serve --http` dynamic-load. Validate over HTTP on **localhost** with the **existing github/jira tools** (`--no-auth`, `KUZO_DEV`-gated). **Exit:** an MCP client completes `initialize` + a github/jira tool call over Streamable HTTP; default `pnpm install` still pulls **zero** HTTP/OAuth deps.
- **4b — OAuth + claude.ai reachability + host it.** OAuth Resource Server (`requireBearerAuth` + `mcpAuthMetadataRouter` + RFC 8707 audience verify, `§6.3`); the **Authorization-Server decision** (`§6.2` — managed, DCR/CIMD); Cloudflare Tunnel (**bot-mode off**, `§5`); add the claude.ai connector; **prove OAuth via MCP Inspector before trusting the phone**. **Exit:** claude.ai web/phone reaches the hosted kuzo (github/jira tools), only-Sean. *This is already the customer-facing-MCP proof for the pitch — real plugins, usable from regular Claude.*
- **4c — AppleTV plugin (the POC/demo) on the finished foundation.** The `@kuzo-mcp/types` changes (`§3.3`: network-LAN + `system:exec`) + the `--allow-child-process` wiring (`§3.2`) — needed ONLY here; then `@kuzo-mcp/plugin-appletv` (`§3`), proven over **stdio first** (de-risk pyatv), then it rides the already-built HTTP transport; appletv-only hosted instance. **Exit:** AppleTV controllable from Sean's phone via Claude.

QA (the `0.1.0` milestone) follows 4c. **Multi-tenant** (many customers, per-tenant auth/isolation) is the additional layer the pitch needs *beyond* this single-user foundation — out of scope here, but 4a/4b build ~70% of its plumbing.
...
### 3.8 Now-playing
4c ships a **snapshot tool** (single `getState()`), not a live subscription. The EventEmitter push/streaming model is later work. **Prompt-injection:** now-playing/app-name text is untrusted — fence it as data in tool output, never as instructions.
...
### 6.2 Authorization Server — DECISION POINT (resolve in 4b, criteria now)
...
**Recommendation:** 4a uses (D); 4b adopts (A) with a DCR/CIMD-verified managed AS scoped to Sean. This is the biggest open risk, deliberately isolated to 4b.
...
### 6.4 Other controls
- Bind loopback; expose only via the tunnel; DNS-rebinding protection on; rate-limit. **CORS Origin is browser-only — not a security boundary** for the headless broker (keep it permissive enough for the MCP Inspector during 4b).
...
## 7. `@kuzo-mcp/types` & contract changes (needed for 4c — the AppleTV plugin only; the 4a/4b transport foundation does NOT touch the capability system)
...
3. No new `AuditAction` expected (control/read, not credential mutation) — confirm in 4c.

---

## 8. Acceptance criteria
- **4a (transport foundation):** default `pnpm install` pulls **no** HTTP/OAuth deps; the `bootKuzo`/`buildMcpServer` refactor leaves the stdio path byte-behavior-identical (2.5e parity green); `kuzo serve --http` without the package prints the install hint (and rethrows non-MNF errors); with it, a client completes `initialize` + a **github/jira** tool call over Streamable HTTP on loopback; DNS-rebinding protection active; session 200/404/400 behave per spec; **`--no-auth` refuses to boot without `KUZO_DEV=1` and refuses a non-loopback host**.
- **4b (OAuth + host):** bot protection disabled on the zone; **OAuth + audience-verify proven via MCP Inspector/curl against the public URL FIRST**; then claude.ai web/phone reaches the hosted kuzo's github/jira tools, only-Sean; a wrong-audience token → 401; **the publicly-exposed instance uses scoped/throwaway credentials, never real dev tokens**.
- **4c (AppleTV POC):** types prereq lands (`NetworkCapability` LAN + `system:exec`→`--allow-child-process`; `check:manifest` + parity green); pyatv-absent → skip with clear warning (server boots); under `KUZO_NODE_PERMISSIONS=true` the exec-declaring plugin's spawns succeed; paired device → nav/playback/launch-app/list-apps work over stdio **and** over the built HTTP transport; now-playing via a **single** `getState()`; every command has a 10s timeout + 1-in-flight mutex; unreachable device → Claude-readable error (no hang); **an injected `'error'` does NOT restart-loop the child**; **no orphaned `atvremote`/`atvscript` after shutdown or N induced crash-respawns**; `typecheck`/`lint`/`build` + `node:test` (resolver + error mapping) green; then AppleTV controllable from Sean's phone.
...
- **Authorization Server complexity + DCR availability** (`§6.2`) — the real unknown; isolated to 4b.
...
1. AS choice + DCR/CIMD verification per candidate (4b).
2. `appletv_power` protocol dependency (Companion-only vs needs-AirPlay) — confirm in 4c.
3. CIMD client_id-document constraints, if CIMD over DCR (4b verify).
...
## 11. Phase placement
Phase 4 ("New Integrations") gains **two** things: the **remote-transport capability** (`@kuzo-mcp/server-http` + the `bootKuzo` seam) and **AppleTV** as its first integration. Add both to the `docs/PLANNING.md` Phase 4 section (currently confluence/discord/calendar/sms/…). Sequence: **4a** (transport foundation, validated with github/jira) → **4b** (OAuth + host + claude.ai connector) → **4c** (AppleTV: the `@kuzo-mcp/types` changes + the plugin) → QA (`0.1.0`).

...
11. **Release strategy:** changesets bump all 6 packages on **PATCH** (per the project-wide pre-1.0 versioning cadence in `CLAUDE.md` / `docs/PLANNING.md`). Phase 2.6 is still pre-QA — `0.1.0` is reserved for "first version we trust enough for our own daily use," claimed AFTER `kuzo serve` lands AND we've personally used it for a stretch. The 2.6 ship itself is not that milestone; we'd want at least a few weeks of dogfood first.
    - `@kuzo-mcp/types` bumps `0.0.1 → 0.0.2`. New runtime exports: `isCredentialCapability` type guard. New optional manifest field: `KuzoPluginV2.testCredential`.
    - `@kuzo-mcp/core` bumps `0.0.2 → 0.0.3`. New exports + new boot sequence. **Breaking export rename**: `AuditLogger` (concrete class) → `AuditLogger` (interface) + new `FileBackedAuditLogger` (concrete class). Any external consumer doing `new AuditLogger(...)` must switch to `new FileBackedAuditLogger(...)`. Pre-1.0 means every patch can break — call out in CHANGELOG body but no semver-minor signal required.
    - `@kuzo-mcp/cli` bumps `0.0.2 → 0.0.3` (new `credentials` + `serve` subcommand trees).
    - Three plugins bump `0.0.2 → 0.0.3` — `kuzoPlugin.capabilities` is a new contract surface. Document the cross-version-mix gotcha (`@kuzo-mcp/core@0.0.3` reads `kuzoPlugin.capabilities` statically; `@kuzo-mcp/plugin-*@0.0.2` doesn't carry the field) in CHANGELOG so users mix-pinning don't get bitten silently.
    ...
    - **First minor bump to `0.1.0` is the post-QA milestone** (separate event): after credentials phase ships AND `kuzo serve` survives a stretch of personal daily use. Add a CHANGELOG entry then that retroactively summarizes what 0.0.x earned across phases 2-2.6.

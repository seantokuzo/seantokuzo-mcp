...
7. **Keychain-binding (and adjacent crypto-lib) supply chain compromise.** The keychain binding (`@napi-rs/keyring` and its Rust transitive deps — and `age`/`sops` if Tier 2 lands) loads into the kuzo-mcp process and mediates **every** credential read. Blast radius on compromise is total. Sigstore + Trusted Publishing covers `@kuzo-mcp/*` packages but **does not** cover their third-party deps. Part A's library choice must therefore land with a concrete dep-pinning + version-bump-review policy, not just "use `@napi-rs/keyring`."

Vector 2 is the one that genuinely blocks third-party plugin distribution. Vectors 1, 3, 4 are the user-visible "your tokens aren't sitting in plaintext" promise that has positioning value. Vector 7 is the structural risk we accept by depending on a native crypto/keychain library at all — the spec session must price it in, not hide it.
...
3. **Linux headless / CI fallback.** Encrypted-file-with-passphrase, encrypted-file-with-`KUZO_PASSPHRASE`-env, plain env-var fallback with audited warning, refuse-to-start. Recommended: explicit `KUZO_CRED_BROKER=env` opt-in with startup banner + audit entry. **Caveat the spec session must weigh:** plain-env fallback leaves the token in `/proc/$PID/environ` (Linux) / equivalent (macOS) for the **entire server lifetime**, readable by any process running as the same user. This is strictly worse than the `ps aux` / argv leak named in §2 vector 5 (env is durable; argv can be overwritten). Specifically against vector 1 (local malware), `KUZO_PASSPHRASE`-on-disk-encrypted-file may be the better default than plain-env-with-warning.
4. **At-rest encryption when keychain available.** Always-keychain (Tier 1 only) vs hybrid (Tier 2 ciphertext file with key in keychain). Recommended: Tier 1 only for v1, hybrid as a fast-follow if backup-leak concerns surface. **If hybrid lands, Part A must mandate an AEAD cipher (AES-256-GCM or ChaCha20-Poly1305) — never unauthenticated AES-CBC.** Without AEAD, an attacker with file-write access can flip ciphertext bits to corrupt or shape decrypted bytes; AEAD also gives tamper-detect for the file-restored-from-old-backup case.
5. **`process.env` scrubbing.** Delete from env after broker reads, leave it, or leave it but log the read. Recommended: delete after read, with a kill-switch env (`KUZO_NO_ENV_SCRUB=1`) for the dotenv-library-collision case. **Two ordering invariants the spec must pin down or vector 2 defense breaks:** (a) **Boot ordering** — broker init + scrub MUST complete before any plugin's `init()` is called; otherwise the first-loaded plugin snapshots `process.env` and exfiltrates pre-scrub. (b) **Child-process inheritance** — `delete process.env.X` only affects subsequently-spawned children; anything spawned before the scrub keeps the inherited env. Spec must require: no child processes spawned before the scrub; or, if some core child is unavoidable, it must be respawned post-scrub with a clean env.
6. **Provisioning UX shape.** `kuzo init` (one-shot first-run) vs `kuzo credentials set <name>` (per-credential) vs inline-with-`plugins install` (prompt right after consent succeeds) vs all three. Recommended: inline-with-install + `kuzo credentials set` for rotation. No `kuzo init` (over-engineered for our current footprint). **Part B must pin the secret-input contract**, not just the command shape (defends §2 vector 5): (a) **No positional arg or flag value for the secret** — ever; `kuzo credentials set GITHUB_TOKEN ghp_xxx…` must not exist. (b) **TTY:** prompt with echo-off (Inquirer `password`-type). (c) **Non-TTY:** stdin pipe behind an explicit `--stdin` flag (`echo $T \| kuzo credentials set NAME --stdin`) — no auto-detect of pipe-vs-tty, because auto-detect makes shell-history leaks possible if `--stdin` is omitted accidentally and the prompt silently reads the pipe. Same contract for `rotate`.
7. **`~/.claude/settings.json` migration.** Hard refusal ("the MCP server refuses to start if `env` block contains a known credential name"), warn-but-continue, or silent migration on first run. Recommended: warn + offer one-shot migration command. **If a migration command lands, Part B must constrain its secret-handling explicitly:** (a) atomic in-place rewrite of `settings.json` with the token redacted — never write the token to a temp/`.bak` file; (b) any error path scrubs the token from the message before logging or audit-emitting; (c) **read-back verify** — after `set`, immediately `get` and byte-compare against the in-memory copy; only proceed with the settings.json rewrite if the read-back matches. Defends against silent-drop failures (keychain backend broken / ACL drift / signed-binary identity mismatch) where `set` returns success but the token is unrecoverable; (d) zero the in-memory copy **only after (c) passes** (success path) or **after the error-path audit-emit completes** (failure path) — zeroing immediately after `set` returns breaks (c) because read-back has nothing left to compare against; (e) audit a `credential.migrated` write entry per Q14 **without** the value. Easy to half-implement and end up with a token in `~/.kuzo/audit.log`, a leftover `.bak`, or — without (c) — a destroyed credential the user can't recover.

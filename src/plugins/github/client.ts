/**
 * GitHubClient — thin Octokit wrapper.
 *
 * No singleton, no global config. Constructed with an explicit token and
 * optional logger by the plugin in `initialize()`.
 */
...
export interface GitHubClientOptions {
  token: string;
  /** Optional default owner (GITHUB_USERNAME). Used by parseRepoIdentifier fallback. */
  username?: string;
  logger?: PluginLogger;
}
...
export class GitHubClient {
  ...
  constructor(options: GitHubClientOptions) {
    ...
    this.defaultOwner = options.username;
  }

...
Fix plugin manifest `version` drift. Plugins now derive their `version` field from their own `package.json` at compile time via a JSON import (`import pkgJson from "../package.json" with { type: "json" }`), so the published `dist/` always declares the same version as npm. Previously all three plugins had `version: "1.0.0"` hardcoded, which collided with the install CLI's `E_VERSION_MISMATCH` safety check when the first real `0.0.1` release shipped — verified Sigstore attestation succeeded but the final install step refused to complete because `manifest.version=1.0.0 !== resolvedVersion=0.0.1`.
